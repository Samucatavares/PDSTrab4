clear all; clc; close all;
    ...
    figure;
    subplot(2,1,1); plot(eixo_tempo_A, sinal_A); title('Sinal Masculino'); xlabel('tempo (s)'); ylabel('Amplitude');
    ...
    subplot(2,1,2); plot(eixo_frequencia_sinal_A, espectro_de_magnitude); title('espectro Sinal Masculino'); xlabel('Freq (Hz)'); ylabel('Amplitude (linear)');

%% PROCESSA SINAL A
    % parte  1: importa sinais e seus valores de taxa de aquisicao Fs (todo arquivo tem que vir a taxa indicando qual foi a Fs usada pra discretizar o sinal)
    [sinal_B, Fs_B] = audioread('Bola_F.wav');
    ...
    % parte 3: plota informacoes sinal B
    figure;
    subplot(2,1,1); plot(eixo_tempo_B, sinal_B); title('Sinal Feminino'); xlabel('tempo (s)'); ylabel('Amplitude');
    espectro_sinal_B = fft(sinal_B);
    eixo_frequencia_sinal_B = eixo_n*(Fs_B/numero_amostras_sinal_B); 
    espectro_de_magnitude = abs(espectro_sinal_B);
    subplot(2,1,2); plot(eixo_frequencia_sinal_B, espectro_de_magnitude);  title('espectro Sinal Masculino'); xlabel('Freq (Hz)'); ylabel('Amplitude (linear)');